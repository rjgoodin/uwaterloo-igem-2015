...
to setup-cells
  ...
  set-default-shape vasculars "leaf" ;; uses stem-looking shape for vasculature
...
;; Cell Procedures
to become-infected
  ;; change global tracking variables for a first infection
  if (not infected?) 
  [
  if (is-cell? self) [set num-infected num-infected + 1     ;; increment counters
    array:set infect-per-leaf leaf (array:item infect-per-leaf leaf + 1)] 
  set infected? true
  set resistant? false
  set color red
  ]
  ;; regardless, pass on viral particle
  set viral-count viral-count + 1
...
to spread-sar
  ...
  [
    let shared-sar sar-level / 10     ;; tenth of the molecules will be shared
    let num-neighbors (count link-neighbors)
    ask link-neighbors
    [
      set sar-level sar-level + shared-sar / num-neighbors
    ...
    [ 
      if (viral-count + mod-viral-count > -1)
        [
          ...
          [
            set viral-count viral-count - 1 ;; take away the one that we will spread
            if any? link-neighbors with [not resistant?] ;; make sure there's a valid target
            [
              ask one-of link-neighbors with [not resistant?] ;; put it into that target
              [ become-infected]
            ]
          ]
        ]
...
to assemble-virus
  ;; viral-count increases with P6 and pregenomes
  ;; p6 increases with copy number
  ;; pregenomes increases with copy number
  ;; copy number increases with viral count
  ;;  -- viral count should be the thing we spread
  ask cells with [infected?]
    [
      ;; 35S RNA formation
